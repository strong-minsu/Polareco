...
const MyInfoPage = () => {
  ...
  const isGoogleUser = useSelector((state: RootState) => state.google.isGoogle);
  ...
  const DayComponent = day.map(day => {
    return (
      <DayHillContainer>
        <DayContainer>
          <Day>{day}</Day>
  ...
  const BadgeComponent = badge.map(badge => {
    return (
      <UserBadgeContainer>
      ...
      axios
        ...
        .catch(error => {
          ...
          if (error.response.status === 403) {
            tokenExpirationHandler(deleteUserHandler);
  ...
  const PasswordModalContent = () => {
    ...
    const changePassword = async () => {
      try {
        ...
        console.log(error);
        if (error.response.status === 403) {
          tokenExpirationHandler(changePassword);
        } else if (error.respose.status === 401) {
          console.log(error);
          alert('현재 비밀번호가 일치하지 않습니다.');
        } else {
    ...
    axios
      ...
      .then(response => {
        if (response.status === 200) {
          ...
          axios
            .patch(`${APIURL}/update/profile-image`, formData, {
              headers: {
                Authorization: USERACCESSTOKEN,
              },
              params: {
  ...
  const updateUsername = async () => {
    ...
    // 서버 업데이트
    try {
      const response = await axios.patch(`${APIURL}/update/username`, tempUsername, {
        headers: {
          Authorization: USERACCESSTOKEN,
        },
        params: {
          newUsername: tempUsername, // 쿼리 파라미터로 username 추가
        },
      });
